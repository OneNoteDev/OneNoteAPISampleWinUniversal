...
namespace OneNoteServiceSamplesWinUniversal.OneNoteApi
{
	...
	internal static class Auth
	{
		...
		internal static async Task<string> GetAuthToken(AuthProvider provider = AuthProvider.WindowsLiveId)
		{
			if (provider == AuthProvider.WindowsLiveId)
			{
		...
		internal static async Task SignOut(AuthProvider provider = AuthProvider.WindowsLiveId)
		{
			if (provider == AuthProvider.WindowsLiveId)
			{
		...
		internal static bool IsSignedIn(AuthProvider provider = AuthProvider.WindowsLiveId)
		{
			if (provider == AuthProvider.WindowsLiveId)
		...
		internal static async Task<string> GetUserName(AuthProvider provider = AuthProvider.WindowsLiveId)
		{
			if (provider == AuthProvider.WindowsLiveId)
			{
